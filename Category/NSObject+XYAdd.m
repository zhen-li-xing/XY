...
@interface _XYBlockTarget : NSObject

/**添加一个KVOBlock*/
- (void)xy_addBlock:(void(^)(__weak id obj, id oldValue, id newValue))block;
- (void)xy_addNotificationBlock:(void(^)(NSNotification *notification))block;

- (void)xy_doNotification:(NSNotification*)notification;

@end

@implementation _XYBlockTarget{
    //保存所有的block
...
static void *const XYKVOBlockKey = "XYKVOBlockKey";
static void *const XYKVOSemaphoreKey = "XYKVOSemaphoreKey";

- (void)xy_addObserverBlockForKeyPath:(NSString*)keyPath block:(void (^)(id obj, id oldVal, id newVal))block {
    if (!keyPath || !block) return;
    dispatch_semaphore_t kvoSemaphore = [self _xy_getSemaphoreWithKey:XYKVOSemaphoreKey];
    dispatch_semaphore_wait(kvoSemaphore, DISPATCH_TIME_FOREVER);
    //取出存有所有KVOTarget的字典
    NSMutableDictionary *allTargets = objc_getAssociatedObject(self, XYKVOBlockKey);
    NSLog(@"xy_addObserverBlockForKeyPath   allTargets--- %@",allTargets);

    if (!allTargets) {
        //没有则创建
        allTargets = [NSMutableDictionary new];
        //绑定在该对象中
        objc_setAssociatedObject(self, XYKVOBlockKey, allTargets, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    //获取对应keyPath中的所有target
    _XYBlockTarget *targetForKeyPath = allTargets[keyPath];
    if (!targetForKeyPath) {
        //没有则创建
        targetForKeyPath = [_XYBlockTarget new];
        //保存
    ...
    [targetForKeyPath xy_addBlock:block];
    //对第一次注册KVO的类进行dealloc方法调剂
    [self _xy_swizzleDealloc];
    dispatch_semaphore_signal(kvoSemaphore);
}
- (void)xy_removeObserverBlockForKeyPath:(NSString *)keyPath{
    if (!keyPath.length) return;
    NSMutableDictionary *allTargets = objc_getAssociatedObject(self, XYKVOBlockKey);
    NSLog(@"xy_removeObserverBlockForKeyPath  allTargets--- %@",allTargets);

    if (!allTargets) return;
    _XYBlockTarget *target = allTargets[keyPath];
    if (!target) return;
    dispatch_semaphore_t kvoSemaphore = [self _xy_getSemaphoreWithKey:XYKVOSemaphoreKey];
    dispatch_semaphore_wait(kvoSemaphore, DISPATCH_TIME_FOREVER);
...
- (void)xy_removeAllObserverBlocks {
    NSMutableDictionary *allTargets = objc_getAssociatedObject(self, XYKVOBlockKey);
    NSLog(@"xy_removeAllObserverBlocks   allTargets--- %@",allTargets);

    if (!allTargets) return;
    dispatch_semaphore_t kvoSemaphore = [self _xy_getSemaphoreWithKey:XYKVOSemaphoreKey];
    dispatch_semaphore_wait(kvoSemaphore, DISPATCH_TIME_FOREVER);
    [allTargets enumerateKeysAndObjectsUsingBlock:^(id key, _XYBlockTarget *target, BOOL *stop) {
...
static void *const XYNotificationBlockKey = "XYNotificationBlockKey";
static void *const XYNotificationSemaphoreKey = "XYNotificationSemaphoreKey";

- (void)xy_addNotificationForName:(NSString *)name block:(void (^)(NSNotification *notification))block {
    if (!name || !block) return;
    dispatch_semaphore_t notificationSemaphore = [self _xy_getSemaphoreWithKey:XYNotificationSemaphoreKey];
    dispatch_semaphore_wait(notificationSemaphore, DISPATCH_TIME_FOREVER);
    NSMutableDictionary *allTargets = objc_getAssociatedObject(self, XYNotificationBlockKey);
    NSLog(@"xy_addNotificationForName  allTargets--- %@",allTargets);
    if (!allTargets) {
        allTargets = @{}.mutableCopy;
        objc_setAssociatedObject(self, XYNotificationBlockKey, allTargets, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    _XYBlockTarget *target = allTargets[name];
    if (!target) {
        target = [_XYBlockTarget new];
        allTargets[name] = target;
        [[NSNotificationCenter defaultCenter] addObserver:target selector:@selector(xy_doNotification:) name:name object:nil];
    }
    [target xy_addNotificationBlock:block];
    [self _xy_swizzleDealloc];
    dispatch_semaphore_signal(notificationSemaphore);
    
}

- (void)xy_removeNotificationForName:(NSString *)name{
    if (!name) return;
    NSMutableDictionary *allTargets = objc_getAssociatedObject(self, XYNotificationBlockKey);
    NSLog(@"xy_removeNotificationForName   allTargets--- %@",allTargets);
...
- (void)xy_removeAllNotification{
    NSMutableDictionary *allTargets = objc_getAssociatedObject(self, XYNotificationBlockKey);
    NSLog(@"xy_removeAllNotification   allTargets--- %@",allTargets);

    if (!allTargets.count) return;
    dispatch_semaphore_t notificationSemaphore = [self _xy_getSemaphoreWithKey:XYNotificationSemaphoreKey];
    dispatch_semaphore_wait(notificationSemaphore, DISPATCH_TIME_FOREVER);
    [allTargets enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, _XYBlockTarget *target, BOOL * _Nonnull stop) {
        [[NSNotificationCenter defaultCenter] removeObserver:target];
    }];
    [allTargets removeAllObjects];
    dispatch_semaphore_signal(notificationSemaphore);
...
- (void)xy_postNotificationWithName:(NSString *)name userInfo:(NSDictionary *)userInfo{
...
- (void)_xy_swizzleDealloc{
    ...
    @synchronized(swizzleClass) {
        ...
        id newDealloc = ^(__unsafe_unretained id objSelf){
            //在这里我们移除所有的KVO
            [objSelf xy_removeAllObserverBlocks];
            //移除所有通知
            [objSelf xy_removeAllNotification];
...
- (dispatch_semaphore_t)_xy_getSemaphoreWithKey:(void *)key{
