...
- (void)showNumberBadgeWithValue:(NSInteger)value
{
    ...
    if (value > self.badgeMaximumBadgeNumber) {
        frame.size.width = BADGE_WIDTH + 10;
        self.badge.text = [NSString stringWithFormat:@"%@+",@(self.badgeMaximumBadgeNumber)];
    }else{
        if (value > 9) {
            frame.size.width = BADGE_WIDTH + 5;
        }
...
//        CGFloat redotWidth = 8;
        ...
        CGFloat width = (WIDTH_L - 20)/3;
        int x = width/2 + BADGE_WIDTH/3;
        CGRect frm = CGRectMake(x, -2, BADGE_WIDTH, BADGE_WIDTH);
        NSLog(@"brdge : %@",NSStringFromCGRect(frm));
